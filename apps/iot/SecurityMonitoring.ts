...
export interface SecurityMonitoringEvents {
  initialized: { timestamp: string; config: SecurityMonitoringConfig };
  executed: { result: any; duration: number; params: any };
  error: { error: Error; context: any; timestamp: string };
  configUpdated: { oldConfig: SecurityMonitoringConfig; newConfig: SecurityMonitoringConfig };
...
export class SecurityMonitoring {
  ...
  private listeners: Map<keyof SecurityMonitoringEvents, Function[]> = new Map();
  ...
  public on<K extends keyof SecurityMonitoringEvents>(
    event: K, 
    callback: (data: SecurityMonitoringEvents[K]) => void
  ): void {
  ...
  private emit<K extends keyof SecurityMonitoringEvents>(
    event: K, 
    data: SecurityMonitoringEvents[K]
  ): void {
