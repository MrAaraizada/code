...
export interface SecureStorageV2Config {
  enabled: boolean;
  version: string;
...
export class SecureStorageV2 {
  private config: SecureStorageV2Config;
  private initialized: boolean = false;

  constructor(config: SecureStorageV2Config) {
    this.config = {
      ...config,
      metadata: {
  ...
  public getConfig(): SecureStorageV2Config {
    return { ...this.config };
  }
}
